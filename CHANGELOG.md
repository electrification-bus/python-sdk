...
- `.ebus-spec.json` re-synced against the specification (HEAD `6e582c9`, 2026-08-01), which it had drifted 48 commits behind: framework 0.5 -> 0.7, `capabilities/info` 0.1 -> 0.2, `capabilities/meter` 0.1 -> 0.2, `devices/utility-meter` 0.3 -> 0.6, `registries/capability-types` 0.11 -> 0.19. The `supports` list is unchanged: no framework feature was added or removed across 0.5 -> 0.7. Two substantive corrections beyond the version numbers: `grid`, `status`, `demand` and `power-quality` are now pinned directly, because utility-meter 0.6 split them out of the utility-meter data model into standalone catalogs and the old `notes` rationale ("covered by pinning utility-meter 0.3") therefore no longer held; and `soc` 0.1 is pinned, the customizer having first-class knowledge of it. None of the version bumps required SDK code changes: each is additive-MAY or prose. Reported by [@cayossarian](https://github.com/cayossarian), who ran the specification's `drift-report.py` against us. ([#27](https://github.com/electrification-bus/python-sdk/issues/27))
